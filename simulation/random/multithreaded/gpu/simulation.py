...
class BJRGraphSimulation:
    ...
    def process_edges(self, chunk):
        """Process edges for a given chunk of nodes using GPU."""
        updates = {"add": [], "remove": []}
        
        node_list = list(self.graph.keys())
        
        # Prepare probability arrays for GPU (use CuPy)
        p_gpu = cp.full((len(node_list), len(node_list)), self.p)
        q_gpu = cp.full((len(node_list), len(node_list)), self.q)
        random_vals_add = cp.random.rand(len(node_list), len(node_list))
        random_vals_remove = cp.random.rand(len(node_list), len(node_list))

        for i in chunk:
            for j in range(len(node_list)):
                if i != j:
                    # Add edges with probability `p` (on GPU)
                    if random_vals_add[i, j] < p_gpu[i, j] and j not in self.graph[node_list[i]]:
                        updates["add"].append((node_list[i], node_list[j]))

                    # Remove edges with probability `q` (on GPU)
                    if random_vals_remove[i, j] < q_gpu[i, j] and j in self.graph[node_list[i]]:
                        updates["remove"].append((node_list[i], node_list[j]))
